...
void propagate(ll m, ll b, ll e)
{
    ...
    if (b != e)
        ...
        lazy[2 * m] = lazy[m], lazy[2 * m + 1] = lazy[m], p[2 * m] = true, p[2 * m + 1] = true;
...
ll query(ll m, ll b, ll e, ll l, ll r)
{
    ...
    if (l == b && r == e)
        return t[m];
    ll mid = (b + e) / 2;
    return min(query(2 * m, b, mid, l, min(mid, r)), query(2 * m + 1, mid + 1, e, max(l, mid + 1), r));
}
int main()
{
    op()
